...
- 默认 `Option + S` 和菜单栏“截图翻译”改为直接进入原位覆盖链路，框选后自动 OCR、翻译并回填，不再打开文字悬浮窗等待二次确认。
- 覆盖窗口会立即显示冻结原图和处理状态；每批译文返回后即时回填，用户无需等待全部批次结束。
- macOS 15 及以上优先接入 Apple `TranslationSession` 设备端批量翻译；macOS 26.4 及以上使用 `lowLatency` 策略，语言包安装后不产生 API 成本。
- 首次缺少语言包时由 macOS 请求一次下载许可；用户取消或本地翻译失败时不静默上传截图文字，错误直接保留在覆盖窗口。
- macOS 13–14 的云端兼容路径从逐批串行改为最多两批并发，在控制请求压力的同时缩短多批截图的等待时间。
- 关闭覆盖窗口会取消仍在进行的 OCR 或翻译任务；原文字翻译链路保留为菜单栏辅助入口。

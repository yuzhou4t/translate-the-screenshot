...
- 所有截图快捷键现在都会先冻结各显示器画面，再让用户在静态画面上拖动选择区域，避免框选期间页面或视频内容继续变化。
- 框选层增加选区外暗化、实时尺寸、操作提示、双层描边和角点反馈，并继续支持 `Esc` 取消与按住空格移动选区。
- 标注工具栏改为图标分组与强调色主按钮，复制操作优先放在选区下方；底部空间不足时优先放入选区底部，最后才回退到上方。
- 选区完成后默认进入移动模式：内部拖动可整体移动，四边和四角拖动可重新调整尺寸，并支持方向键微调及 macOS 三指拖移。
- 在矩形、箭头和马赛克等标注工具中长按约 0.32 秒可临时移动选区；每次移动或缩放都会从原冻结画面重新裁切，而不是拉伸旧截图。
- 冻结画面按显示器独立裁切，保留 Retina 像素尺寸，并覆盖负坐标副屏、移动/缩放边界与屏幕裁切回归检查。

### 三条独立图片翻译快捷键

- `Option + W` 改回本地坐标链路：macOS 15 及以上使用“Apple Vision OCR/坐标 → Apple 系统翻译 → 原坐标回填”，macOS 13–14 使用默认服务兼容翻译。
- API 高质量坐标翻译和火山图片翻译各新增独立、可编辑的快捷键槽；默认留空，避免与已有用户快捷键冲突。
- 旧 `screenshotTranslateOverlay` 快捷键持久化键继续归本地坐标翻译使用，保留已有自定义或清除状态。
- 当前配置的默认 Provider、Endpoint、模型、Key 与全局备用服务会被直接复用，不新增场景配置。
- 坐标翻译窗口新增“API 高质量重译”，可覆盖 Apple 本地译文；API 失败时保留已有可用结果或原文。
- API 坐标翻译尊重服务商配置的超时时间，不再强制压缩为 12 秒。
- 火山整图上传可从菜单或其独立快捷键主动启动，并继续执行首次上传同意和月度用量保护。

### 轻量化与截图到剪贴板

- 新增 `Ctrl + A` 截图到剪贴板：框选后可使用矩形、箭头、文字和马赛克，并支持撤销、取消与复制。
- 截图到剪贴板全程只在内存中处理，不保存文件、不运行 OCR 或翻译，也不发起网络请求。
- 复制的 PNG 会保留 Retina 逻辑尺寸，马赛克强度也会随屏幕缩放比例保持一致。
- 移除历史记录与收藏的界面和持久化；已有 `history.json` / `favorites.json` 不会被自动删除。
- 普通翻译界面收敛为快速、准确和自然三种模式；设置收敛为全局默认服务与一个备用服务，不再暴露按场景路由。
- 删除未使用的远端 Vision 图片 payload 路径；现有翻译服务商、火山整图与本地坐标两条覆盖路径，以及划词、输入、截图和图片 OCR 入口继续保留。
- 统一修复备用服务状态；OCR AI 修复在默认服务不支持 Prompt 时可使用兼容的全局备用服务。

### 本地坐标翻译重复调用修复

- 修复同一语言组合首次翻译完成后，第二次调用可能停滞并在 12 秒后超时的问题；每次请求都会使 Apple 翻译配置失效并触发新的翻译任务。

## 2026-07-29

### 火山图片翻译成为 `Option + W` 默认路径

- `Option + W` 改为直接调用火山 `TranslateImage` 整图翻译接口，框选后立即显示原图并在同一窗口等待云端结果，不再先运行本地 OCR。
- 第一次使用会询问是否允许上传完整截图；同意后不再逐次确认，可在设置中撤销。
- 增加自然月 100 次安全上限：提交前查询火山账号图片用量并与本机保守计数取较大值；请求提交即计数，失败或超时不返还，第 101 次直接阻止，且不自动重试。
- 设置页为火山图片翻译提供 AccessKey ID、Secret Access Key、Region、当月用量和上传许可管理；密钥仅保存在 macOS Keychain。
- 完整截图上传锁定火山官方 HTTPS 域名；账号用量查询失败时停止提交，避免在额度不明时继续产生费用。
- Apple Vision 坐标覆盖翻译保留为菜单栏和结果窗口中的手动备用，不在火山失败时静默触发。
- 火山返回图片默认只驻留当前窗口内存；翻译历史与应用临时截图继续执行 3 天自动清理。

### 截图快捷键、等待上限与三天保留

- 恢复 `Option + S` 为原截图文字翻译，在文字悬浮窗显示结果；截图覆盖翻译调整为 `Option + W`。
- Apple 本地翻译增加“连续 12 秒无结果”上限，每返回一段译文就重新计时；语言包下载尚未完成时停止无限转圈，并提示用户在系统设置完成下载后重试。
- 截图覆盖翻译历史、应用临时截图与 overlay debug 产物只保留 3 天，应用启动或再次使用覆盖翻译时自动清理。
- 用户主动保存的 PNG 和收藏夹内容不参与自动清理。

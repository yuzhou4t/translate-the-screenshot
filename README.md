...
| 截图覆盖翻译 | `Option + W` | 框选后自动 OCR、翻译并原位回填 |
...
7. 本地语言包未就绪或翻译停滞时，连续 12 秒没有结果就停止转圈；每返回一段译文会重新计时。
8. 译文返回后立即在原图位置回填，不需要再次确认。
9. 用户可复制图片、保存 PNG 或查看 debug 输出。
...
截图覆盖翻译历史、应用临时截图和 overlay debug 产物只保留 3 天，应用启动或再次使用覆盖翻译时自动清理。用户主动选择“保存 PNG”导出的文件不属于应用临时数据，不会被自动删除；收藏夹中的显式收藏也继续保留。
